...
describe('Exam paper security (e2e)', () => {
  ...
  const PAPER_LEAKS = ['maths-final.pdf', 'sha256', '"paper"', 'hasPaper', '/paper', '%PDF'];
  ...
  async function world() {
    ...
    await prisma.parentStudent.create({
      data: { parentId: parentProfile.id, studentId: cls.students[0].profile.id },
    });
    ...
    const bAdmin = await createTestUser({ role: Role.SCHOOL_ADMIN, schoolId: b.school.id });
    ...
    return { cls, admin, parentUser, other, b, tokens, examId, subjectId, paperPath };
  }

  const upload = (path: string, token: string, body = pdf, filename = 'maths-final.pdf', contentType = 'application/pdf') =>
    api().put(path).set(bearer(token)).attach('paper', body, { filename, contentType });

  async function submitAndPublish(w: Awaited<ReturnType<typeof world>>) {
    await api().post(`/api/exams/${w.examId}/submit`).set(bearer(w.tokens.teacher)).expect(201);
    await api().post(`/api/exams/${w.examId}/publish`).set(bearer(w.tokens.admin)).expect(201);
  }
  ...
  it('TEST 2: the author uploads a PDF privately and can read it back', async () => {
    ...
    expect(res.body).toMatchObject({ fileName: 'maths-final.pdf', sizeBytes: pdf.length, replaced: false });

    const row = await prisma.examPaper.findUniqueOrThrow({ where: { examId: w.subjectId } });
    expect(row.schoolId).toBe(w.cls.school.id);
    expect(row.sha256).toMatch(/^[0-9a-f]{64}$/);

    const read = await api().get(w.paperPath).set(bearer(w.tokens.teacher)).buffer(true).expect(200);
  ...
  it('rejects non-PDF content, a disguised PDF and oversize files', async () => {
    const w = await world();
    await upload(w.paperPath, w.tokens.teacher, Buffer.from('hello'), 'notes.txt', 'text/plain').expect(400);
    await upload(w.paperPath, w.tokens.teacher, Buffer.from('MZ\x90\x00 not a pdf'), 'virus.pdf').expect(400);
    await upload(w.paperPath, w.tokens.teacher, Buffer.alloc(10 * 1024 * 1024 + 1, 0x25)).expect(413);
  ...
  it('TEST 3 & 4: students and parents get 403 on the paper, before and after publication', async () => {
    ...
    await api().get(w.paperPath + childQuery).set(bearer(w.tokens.parent)).expect(403);
  ...
  it('TEST 16 & 17: other schools, the platform admin and tampered ids are all refused', async () => {
    ...
    await api().get(`/api/exams/${w.examId}/subjects/${randomUUID()}/paper`).set(bearer(w.tokens.admin)).expect(404);
    await api().get(`/api/exams/${randomUUID()}/subjects/${w.subjectId}/paper`).set(bearer(w.tokens.admin)).expect(404);
    await api().get(`/api/exams/${w.examId}/subjects/not-a-uuid/paper`).set(bearer(w.tokens.admin)).expect(400);
    ...
    for (const deadline = Date.now() + 3000; !audit && Date.now() < deadline; ) {
      audit = await prisma.auditLog.findFirst({ where: { action: 'EXAM_PAPER_VIEW', actorUserId: w.admin.id } });
  ...
  it('never exposes the paper in student/parent responses or notifications', async () => {
    ...
    const bodies = [
      (await api().get(`/api/exams/${w.examId}`).set(bearer(w.tokens.student)).expect(200)).text,
      (await api().get('/api/exams').set(bearer(w.tokens.student)).expect(200)).text,
      (await api().get(`/api/exams/${w.examId}?studentId=${studentId}`).set(bearer(w.tokens.parent)).expect(200)).text,
      (await api().get(`/api/exams?studentId=${studentId}`).set(bearer(w.tokens.parent)).expect(200)).text,
    ];
    ...
    while (Date.now() < deadline) {
      rows = await prisma.notification.findMany({ where: { type: 'EXAM_PUBLISHED' } });
  ...
  it('lets only the author replace or remove the paper, and only before review', async () => {
    const w = await world();
    await upload(w.paperPath, w.tokens.teacher).expect(200);
    const first = await prisma.examPaper.findUniqueOrThrow({ where: { examId: w.subjectId } });

    const replaced = await upload(w.paperPath, w.tokens.teacher, Buffer.concat([pdf, Buffer.from('% v2\n')])).expect(200);
    expect(replaced.body.replaced).toBe(true);
    const second = await prisma.examPaper.findUniqueOrThrow({ where: { examId: w.subjectId } });
  ...
  it('the principal views each subject paper while it waits for review, and the teacher replaces one after changes', async () => {
    ...
    const physics = await prisma.subject.create({ data: { schoolId: w.cls.school.id, name: 'Physics' } });
    const physicsSection = await prisma.sectionSubject.create({
      data: { sectionId: w.cls.section.id, subjectId: physics.id, teacherId: w.cls.teacherProfile.id },
    });
    const added = await api()
      .post(`/api/exams/${w.examId}/subjects`)
      .set(bearer(w.tokens.teacher))
      .send({ sectionSubjectId: physicsSection.id, heldAt: '2026-10-13', startMin: 540, endMin: 660, maxScore: 50, passingMarks: 20 })
      .expect(201);
    const second = added.body.subjects.find((s: { sectionSubjectId: string }) => s.sectionSubjectId === physicsSection.id);
    ...
    await upload(w.paperPath, w.tokens.teacher, mathsPdf, 'maths.pdf').expect(200);
    await upload(secondPath, w.tokens.teacher, physicsPdf, 'physics.pdf').expect(200);
    await api().post(`/api/exams/${w.examId}/submit`).set(bearer(w.tokens.teacher)).expect(201);

    // Exam Approvals: the pending detail offers the principal a view, never a change.
    const detail = await api().get(`/api/exams/${w.examId}`).set(bearer(w.tokens.admin)).expect(200);
    expect(detail.body.status).toBe('PENDING_REVIEW');
    expect(detail.body.permissions).toMatchObject({ canViewPaper: true, canManagePaper: false, canReview: true });
    const namesBySubject = Object.fromEntries(
      detail.body.subjects.map((s: { id: string; paper: { fileName: string } | null }) => [s.id, s.paper?.fileName]),
    );
    expect(namesBySubject).toEqual({ [w.subjectId]: 'maths.pdf', [second.id]: 'physics.pdf' });

    // Each subject serves its own bytes, and only through its own examination.
    const readMaths = await api().get(w.paperPath).set(bearer(w.tokens.admin)).buffer(true).expect(200);
    const readPhysics = await api().get(secondPath).set(bearer(w.tokens.admin)).buffer(true).expect(200);
    ...
    await api().get(`/api/exams/${randomUUID()}/subjects/${second.id}/paper`).set(bearer(w.tokens.admin)).expect(404);
    ...
    const replaced = await upload(secondPath, w.tokens.teacher, physicsV2, 'physics-v2.pdf').expect(200);
    expect(replaced.body).toMatchObject({ fileName: 'physics-v2.pdf', replaced: true });

    const afterMaths = await api().get(w.paperPath).set(bearer(w.tokens.admin)).buffer(true).expect(200);
    const afterPhysics = await api().get(secondPath).set(bearer(w.tokens.admin)).buffer(true).expect(200);
  ...
  it('removing a paper or deleting the draft leaves no bytes behind', async () => {
    ...
    await upload(w.paperPath, w.tokens.teacher).expect(200);
    await api().delete(`/api/exams/${w.examId}`).set(bearer(w.tokens.teacher)).expect(200);
    expect(await prisma.examPaper.count()).toBe(0);
