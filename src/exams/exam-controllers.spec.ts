...
describe('exam controllers', () => {
  it.each(
    controllers.flatMap((c) =>
      handlers(c).map((h) => [`${c.name}.${h.name}`, h.fn] as const),
    ),
  )('%s declares @Roles', (_label, fn) => {
  ...
  it('never lets a student or parent reach the exam paper', () => {
    const proto = ExamsController.prototype as unknown as Record<
      string,
      object
    >;
  ...
  it('keeps publishing, review and finalization for the principal only', () => {
    const exams = ExamsController.prototype as unknown as Record<
      string,
      object
    >;
    const results = ExamResultsController.prototype as unknown as Record<
      string,
      object
    >;
    for (const fn of [
      exams.publish,
      exams.requestChanges,
      exams.reject,
      results.finalize,
      results.reopen,
    ]) {
    ...
    expect(Reflect.getMetadata(ROLES_KEY, exams.submit)).toEqual([
      Role.TEACHER,
    ]);
  });
});
