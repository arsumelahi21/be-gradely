/**
 * Times are Int minutes-from-midnight, wall-clock in the school's timezone —
 * the schema's all-Int convention. No Prisma, no I/O, no clock.
 */
...
/**
 * THE conflict primitive (teacher/section/room). Half-open, so touching edges
 * (10:00–10:45 & 10:45–11:30) do NOT conflict; 10:00–10:45 & 10:30–11:15 do.
 */
...
/** Zero-length or reversed ranges are the caller's validation, not this. */
...
/**
 * Pure and deterministic — the service persists the result as the source of
 * truth, so later manual edits survive.
 */
export function generatePeriodSlots(params: {
  ...
  while (cursor < dayEndMin) {
    ...
    // A class period never straddles a break.
    ...
    if (end <= cursor) {
      // No room before the next boundary — advance to it rather than emit a
      // zero-length slot.
      cursor = cap;
      continue;
    }
...
export function validatePeriodSet(
  ...
  // Breaks included: any kind may overlap any other.
